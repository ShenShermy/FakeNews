"""
=============================================================
CDS525 Group Project — Model 3: BERT (standalone)
Fake News Detection
=============================================================
Run order:
    1) python bilstm_fakenews.py
    2) python bert_bilstm_fakenews.py
    3) python bert_fakenews.py          ← this file
...
SEED        = 42        # Keep same across all training scripts!
EPOCHS      = 10
CSV_PATH    = "fakenews.csv"
SAVE_DIR    = "results"
FIG_DIR     = "figures/bert"

# freeze_bert=True  → only train classifier head (fast, lower GPU memory)
...
class BertClassifier(nn.Module):
    """
    BERT → [CLS] token → Dropout → FC → Sigmoid

    Architecture:
        token ids
            ↓
        [BERT Encoder]    → pooler_output (B, 768)  [CLS] representation
            ↓
        [Dropout]
            ↓
        [FC 768 → 1]
    ...
    def forward(self, input_ids, attention_mask):
        # Use [CLS] token's pooled output as sentence representation
...
def run_experiment(X_tr, X_va, X_te, y_tr, y_va, y_te, tokenizer,
    ...
    model     = BertClassifier(freeze_bert=FREEZE_BERT).to(DEVICE)
...
def plot_predictions_table(preds, labels, texts, path, n=100):
    ...
    plt.title(f"BERT Predictions – First {n} Test Samples",
...
if __name__ == "__main__":
    ...
    plot_single(h_bce, "Fig 1 – BERT | BCE Loss (lr=2e-5, bs=16)",
                f"{FIG_DIR}/fig1_bce_baseline.png")

    # ── Fig 2: Focal Loss ─────────────────────────────────
    print("\n" + "="*50 + "\nExp 2 — Focal Loss\n" + "="*50)
    h_focal = exp(FocalLoss(), tag="Focal")
    plot_single(h_focal, "Fig 2 – BERT | Focal Loss (lr=2e-5, bs=16)",
    ...
    plot_multi(bs_hists, "train_loss", "Train Loss",
               "Fig 5 – BERT Train Loss: Different Batch Size",
               f"{FIG_DIR}/fig5_bs_train_loss.png")
    plot_multi(bs_hists, "test_acc", "Test Accuracy",
               "Fig 6 – BERT Test Accuracy: Different Batch Size",
    ...
    with open(f"{SAVE_DIR}/bert_results.json", "w") as f:
        json.dump(save_data, f, indent=2)
    print(f"\n✅ BERT done! Results saved to {SAVE_DIR}/bert_results.json")
